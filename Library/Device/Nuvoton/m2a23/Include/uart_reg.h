...
typedef struct
{
     ...
     * |        |          |If the number of bytes of received data is greater than RX_FIFO (UART_DAT) size, 8 bytes, this bit will be set.
     ...
     * |        |          |The Maximum value shown in RXPTR is 7.
     * |        |          |When the using level of RX FIFO Buffer equal to 8, the RXFULL bit is set to 1 and RXPTR will show 0.
     * |        |          |As one byte of RX FIFO is read by CPU, the RXFULL bit is cleared to 0 and RXPTR will show 7.
     ...
     * |        |          |1 = RX FIFO is full.
     * |        |          |Note: This bit is set when the number of usage in RX FIFO Buffer is equal to 8, otherwise it is cleared by hardware.
     ...
     * |        |          |1 = TX FIFO is full.
     * |        |          |Note: This bit is set when the number of usage in TX FIFO Buffer is equal to 8, otherwise it is cleared by hardware.
     ...
     * |        |          |LIN slave header error detect (SLVHEF (UART_LINSTS[1])),
     * |        |          |LIN slave ID parity error (SLVIDPEF (UART_LINSTS[2])),
     * |        |          |LIN slave header time-out detect (SLVHTOF (UART_LINSTS[4])),
     * |        |          |LIN response time-out detect (RTOUTF (UART_LINSTS[5])),
     ...
     * |[5]     |RTOUTEN   |LIN Response Time-out Detection Enable Bit
     * |        |          |0 = LIN response time-out detection Disabled.
     * |        |          |1 = LIN response time-out detection Enabled.
     * |        |          |Note 1: This bit detects both master/slave response time-out.
     * |        |          |When detecting slave response time-out, SLVEN (UART_LINCTL[0]) and SLVHDEN (UART_LINCTL[1]) must be set to 1.
     * |        |          |Note 2: There would be an internal counter up counting after LIN master sends a header or LIN slave detects a header.
     * |        |          |The master sends header event and response event, and the slave detects header event and receives response event will reset the counter.
     * |        |          |When the counter is equal to or bigger than LINRTOIC (UART_LINRTOUT[23:0]), a LIN response time-out is generated.
     * |        |          |Note 3: In LIN function mode, when detecting response time-out, RTOUTF (UART_LINSTS[5]) flag will be asserted.
     * |        |          |If the LINIEN (UART_INTEN[8]) = 1, an interrupt will be generated.
     ...
     * |        |          |"sync field measure time-out with Automatic Resynchronization mode".
     * |        |          |and 'LIN header reception time-out'.
     ...
     * |[4]     |SLVHTOF   |LIN Slave Header Time-out Flag
     * |        |          |This bit is set by hardware when a LIN header reception time-out is detected in LIN slave mode.
     * |        |          |When this bit is set, SLVHEF (UART_LINSTS[1]) will also be set.
     * |        |          |0 = LIN header time-out not detected.
     * |        |          |1 = LIN header time-out detected.
     * |        |          |Note 1: This bit can be cleared by writing 1 to it.
     * |        |          |Note 2: This bit is only valid when UART is operated in LIN slave mode (SLVEN (UART_LINCTL[0]) = 1) and enables LIN slave header detection function (SLVHDEN (UART_LINCTL[1])).
     * |[5]     |RTOUTF    |LIN Response Time-out Flag
     * |        |          |This bit is set when no LIN response received and the time-out counter equal to or bigger than LINRTOIC (UART_LINRTOUT[23:0]).
     * |        |          |If LINIEN (UART_INTEN[8]) is enabled, the LIN Bus Interrupt will be generated.
     * |        |          |0 = LIN response time-out not detected.
     * |        |          |1 = LIN response time-out detected.
     * |        |          |Note: This bit can be cleared by writing 1 to it.
     ...
     * |        |          |Note 1: This bit can be cleared by writing 1 to it.
     * |        |          |Note 2: This bit is only valid when bit error detection function is enabled (BITERREN (UART_LINCTL [12]) = 1).
     * @var UART_T::BRCOMP
...
#define UART_LINCTL_RTOUTEN_Pos         (5)                                               /*!< UART_T::LINCTL: RTOUTEN Position        */
#define UART_LINCTL_RTOUTEN_Msk         (0x1ul << UART_LINCTL_RTOUTEN_Pos)                /*!< UART_T::LINCTL: RTOUTEN Mask            */
...
#define UART_LINSTS_SLVHTOF_Pos          (4)                                               /*!< UART_T::LINSTS: SLVHTOF Position       */
#define UART_LINSTS_SLVHTOF_Msk          (0x1ul << UART_LINSTS_SLVHTOF_Pos)                /*!< UART_T::LINSTS: SLVHTOF Mask           */

#define UART_LINSTS_RTOUTF_Pos           (5)                                                /*!< UART_T::LINSTS: RTOUTF Position      */
#define UART_LINSTS_RTOUTF_Msk           (0x1ul << UART_LINSTS_RTOUTF_Pos)                  /*!< UART_T::LINSTS: RTOUTF Mask          */
...
#define UART_LINRTOUT_LINRTOIC_Pos       (0)                                               /*!< UART_T::LINRTOUT: LINRTOIC Position    */
#define UART_LINRTOUT_LINRTOIC_Msk       (0xfffffful << UART_LINRTOUT_LINRTOIC_Pos)        /*!< UART_T::LINRTOUT: LINRTOIC Mask        */

#define UART_LINWKCTL_LINWKC_Pos         (0)                                               /*!< UART_T::LINWKCTL: LINWKC Position      */
#define UART_LINWKCTL_LINWKC_Msk         (0xfffffful << UART_LINWKCTL_LINWKC_Pos)          /*!< UART_T::LINWKCTL: LINWKC Mask          */

#define UART_LINWKCTL_SENDLINW_Pos       (24)                                              /*!< UART_T::LINWKCTL: SENDLINW Position    */
#define UART_LINWKCTL_SENDLINW_Msk       (0x1ul << UART_LINWKCTL_SENDLINW_Pos)             /*!< UART_T::LINWKCTL: SENDLINW Mask        */

#define UART_LINWKCTL_LINWKEN_Pos        (28)                                              /*!< UART_T::LINWKCTL: LINWKEN Position     */
#define UART_LINWKCTL_LINWKEN_Msk        (0x1ul << UART_LINWKCTL_LINWKEN_Pos)              /*!< UART_T::LINWKCTL: LINWKEN Mask         */

#define UART_LINWKCTL_LINWKF_Pos         (29)                                              /*!< UART_T::LINWKCTL: LINWKF Position      */
#define UART_LINWKCTL_LINWKF_Msk         (0x1ul << UART_LINWKCTL_LINWKF_Pos)               /*!< UART_T::LINWKCTL: LINWKF Mask          */
