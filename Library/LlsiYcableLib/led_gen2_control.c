...
#define Setup_Period  (HCLK_CLK / 1000000 * 10 / 24)          // HCLK * (10 us / 24 bits)
#define Setup_T1H     ((HCLK_CLK / 1000000 * 10 / 24) << 16)  // HCLK * (10 us / 24 bits)
const uint32_t Setup_LLSI_PERIOD_DUTY[2] = {Setup_Period, Setup_T1H};

#define Data_Period   (HCLK_CLK / 1000000 * 12 / 10)          // HCLK * 1.2 us
#define Data_T1H      ((HCLK_CLK / 1000000 * 9 / 10) << 16)   // HCLK * 0.9 us
#define Data_T0H      ((HCLK_CLK / 1000000 * 3 / 10) << 0)    // HCLK * 0.3 us
...
void LED_Gen2_Transfer_Data(uint8_t target_port)
{
    ...
    PDMA_SetTransferMode(PDMA0, target_port, 32 + target_port, TRUE, (uint32_t)&LED_Gen2_PDMA_DESC[target_port][0]);    // PDMA_LLSI0 = 32
}
...
void LED_Gen2_Control_CMD_ID(uint8_t cmd, uint8_t id)
{
    ...
    *Gen2_Ctrl.fPDMA_Done = 0;
    ...
    /* Start LLSI PDMA transfer */
    PDMA_SetTransferMode(PDMA0, Gen2_Ctrl.Current_Port, 32 + Gen2_Ctrl.Current_Port, TRUE, (uint32_t)&LED_Gen2_PDMA_DESC[Gen2_Ctrl.Current_Port][0]);    // PDMA_LLSI0 = 32
...
void LED_Gen2_LLSI_PDMA_Init(uint8_t target_port)
{
    ...
    /* Change PDMA setting */
    PDMA_SetTransferMode(PDMA0, target_port, 32 + target_port, TRUE, 0);    // PDMA_LLSI0 = 32
    PDMA0->DSCT[target_port].NEXT = (uint32_t)&LED_Gen2_PDMA_DESC[target_port][0] - (PDMA0->SCATBA);
