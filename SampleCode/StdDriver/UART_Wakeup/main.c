...
void UART_DataWakeUp(void)
{
    ...
    /* Set UART data wake-up start bit compensation value.
       ...
       If UART_CLK is selected as HIRC(48MHz) and the HIRC stable time is about 1024 clocks,
       the data wake-up start bit compensation value can be set as 1024. */
    UART1->DWKCOMP = 1024;
...
void UART_RxThresholdWakeUp(void)
{
    ...
    CLK_SetModuleClock(UART1_MODULE, CLK_CLKSEL2_UART1SEL_LXT, CLK_CLKDIV0_UART1(1));
...
void UART_PowerDownWakeUpTest(void)
{
    ...
    printf("Before demo UART wake-up, this demo code will switch HCLK from PLL/2 to HIRC.\n");
