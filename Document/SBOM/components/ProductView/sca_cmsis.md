...
- Upstream source: <https://github.com/ARM-software/CMSIS_5>
...
- `Library/CMSIS/ARM.CMSIS.pdsc`
  - The first `<release>` entry declares package version `5.9.1`

The upstream repository is recorded as a CycloneDX VCS external reference. The
vendored `ARM.CMSIS.pdsc` remains the version authority for this BSP.
...
- `externalReferences[0]`: `type = vcs`, `url = https://github.com/ARM-software/CMSIS_5`
- `licenses[0].license.id`: `Apache-2.0`
- `properties` (recommended custom properties):
  ...
  - `cmsis_package_version = 5.9.1`
  - `bsp:component-origin = third-party`
  - `bsp:component-source = Arm CMSIS`
  - `bsp:evidence-file = Library/CMSIS/ARM.CMSIS.pdsc`
  - `bsp:evidence-path = Library/CMSIS`
  - `bsp:version-evidence = Library/CMSIS/ARM.CMSIS.pdsc (top release: version 5.9.1)`

## 5) Suggested BOM-Ref and purl

Suggested values:

- `bom-ref`: `pkg:generic/cmsis@5.9.1?source=vendored&path=Library/CMSIS`
- `purl`: `pkg:generic/cmsis@5.9.1`
...
```json
{
  ...
  "properties": [
    ...
    { "name": "cmsis_package_version", "value": "5.9.1" },
    { "name": "bsp:component-origin", "value": "third-party" },
    { "name": "bsp:component-source", "value": "Arm CMSIS" },
    { "name": "bsp:evidence-file", "value": "Library/CMSIS/ARM.CMSIS.pdsc" },
    { "name": "bsp:evidence-path", "value": "Library/CMSIS" },
    { "name": "bsp:version-evidence", "value": "Library/CMSIS/ARM.CMSIS.pdsc (top release: version 5.9.1)" }
  ]
}
```
...
- Use `5.9.1` as the CMSIS package version for the top-level SBOM component.
...
- The current `Library/CMSIS` tree does not include the upstream all directories.
